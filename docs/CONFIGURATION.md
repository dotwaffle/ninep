...
| `WithMaxInflight` | `WithMaxInflight(n int)` | `64` | Maximum concurrent in-flight requests per connection. Values less than 1 are clamped to 1. Internally this bounds the size of the per-connection worker pool (lazy-spawn, workers stay alive for the connection lifetime) and the buffered `workCh`. Back-pressure is applied when all workers are busy and the cap is reached. Externally observable semantics are unchanged from earlier versions (bounded concurrency per connection). |
...
| `cleanupDeadline` | `5s` | `server/cleanup.go` | Maximum time to wait for inflight requests to drain during connection cleanup before closing `workCh` and force-exiting. |
