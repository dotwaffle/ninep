...
// Errno represents a Linux errno value on the wire. It is independent of Go's
// syscall.Errno to ensure consistent behavior across platforms and to support
// kernel-internal errno values like ENOTSUPP (524) that are absent from the
// syscall package.
type Errno uint32

// Error returns a human-readable description of the errno.
...
// Is reports whether target matches this errno value. This enables errors.Is
// comparison between Errno values.
func (e Errno) Is(target error) bool {
	t, ok := target.(Errno)
	return ok && e == t
}

// Linux errno constants from errno-base.h (1-34) and errno.h (35-133).
// Values are compile-time constants, not derived from the runtime platform.
const (
	EPERM           Errno = 1
	ENOENT          Errno = 2
	ESRCH           Errno = 3
	...
	ENOTEMPTY       Errno = 39
	ELOOP           Errno = 40
	EWOULDBLOCK     Errno = EAGAIN // Alias.
	ENOMSG          Errno = 42
	EIDRM           Errno = 43
	ECHRNG          Errno = 44
	EL2NSYNC        Errno = 45
	...
	ENOTSUP         Errno = 95
	EOPNOTSUPP      Errno = 95 // Same value as ENOTSUP on Linux.
...
// Kernel-internal errno values not present in the standard errno headers.
const (
	// ENOTSUPP is a kernel-internal errno (524) meaning "operation not
	// supported". It is distinct from ENOTSUP (95). The 9P2000.L protocol
...
// errnoNames maps errno values to human-readable descriptions.
