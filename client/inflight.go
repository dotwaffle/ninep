...
// zero-copy Rread path, an optional caller-supplied dst slice.
//
// Entry-type rationale: a previous design stored chan proto.Message
// directly. Replacing it with a *requestEntry adds one pointer
// indirection per lookup but lets the read loop's Rread fast path
// consult entry.dst without a second map probe - and keeps the
// register / deliver / unregister contract byte-identical for non-ZC
// callers (roundTrip, flushAndWait).
//
// RWMutex-guarded map; read goroutine takes RLock for the per-frame
// lookup; caller goroutines take Lock for register / unregister pairs.
// Callers MUST register BEFORE writing the T-message and MUST
// unregister BEFORE returning the tag to the allocator.
type inflightMap struct {
	mu      sync.RWMutex
	entries map[proto.Tag]*requestEntry
}

// requestEntry is the inflight map's value type. The indirection
// through *requestEntry adds the optional dst slice + n out-field for
// the zero-copy Rread path.
//
// Invariants:
//
...
// goroutine's send in deliver never blocks.
//
// Callers (roundTrip / flushAndWait) see no behavior change. The
// *requestEntry indirection is internal.
...
// If the server returns more than len(dst) bytes, the read loop treats
// it as a protocol error and shuts the Conn down rather than silently
// truncating.
//
...
// deliver posts msg to the chan registered under tag. Non-blocking per
// the cap-1 + tag-serialization invariant. If the tag was not
// registered (late response after caller ctx cancel or shutdown race,
// or a misbehaving server), msg is returned to its R-message cache
// via putCachedRMsg and then dropped - caller logs upstream if
// desired.
//
// The send happens under RLock to serialize against cancelAll's
// close(ch) call. Without this, the -race detector flags (correctly)
// that chansend and closechan may be concurrent on the same channel
// when the read loop delivers a message while Close is tearing down.
// The RLock is released immediately after the non-blocking select so
// deliver never parks while holding the lock.
//
// Cache reclamation on the drop paths: without this, every
// cancellation / shutdown race where the server's reply beat the
// caller's unregister leaked a cached Rread/Rwalk/Rlerror/etc. to GC
// and slowly drained the bounded per-type caches. The cross-package
// dependency inflight -> msgcache is accepted here rather than
// plumbed as a callback, because the cache is a package-private
// implementation detail of the same package.
func (im *inflightMap) deliver(tag proto.Tag, msg proto.Message) {
	...
	case entry.ch <- msg:
	default:
		// Unreachable under correct usage - cap-1 chan +
		// tag-serialization (free-list handoff) means the slot is free.
		// Defense-in-depth for Tflush late-delivery scenarios.
		putCachedRMsg(msg)
	}
}
...
// cancelAll closes every registered chan and drops the map to empty.
// Called during Conn shutdown via signalShutdown. After cancelAll, any
// caller blocked on <-respCh unblocks with (zero proto.Message,
// ok=false); callers detect !ok + the Conn's closed state and return
// ErrClosed.
