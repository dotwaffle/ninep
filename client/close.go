...
//
// Close does NOT return while caller or read goroutines are still
// running (no background reaping). Safe to call from multiple
// goroutines; idempotent via closeOnce.
//
...
// behavior. If ctx has a Deadline, the remaining time is used as the
// drain bound; otherwise the default 5s applies. Named after
// http.Server.Shutdown for idiom parity.
//
// An already-cancelled ctx (ctx.Err() != nil) skips the drain but still
// waits for the read goroutine to exit, preserving the invariant that
// no goroutines leak past Shutdown's return.
...
// same callerWG.Wait / readerWG.Wait. readerWG.Wait runs unconditionally
// before return - no background reaping.
func (c *Conn) shutdown(ctx context.Context, timeout time.Duration) error {
	...
	//
	// Do not return to the caller while reaping in the background -
	// this Wait is the load-bearing invariant of Close.
	c.readerWG.Wait()
	return nil
}
