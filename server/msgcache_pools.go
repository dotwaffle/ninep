...
//
// Cached types mirror the pre-extraction set:
//   - Tread, Twrite (data I/O - bulk of traffic)
//   - Twalk, Tclunk (every open/close cycle)
//   - Tlopen, Tgetattr (file metadata lifecycle)
//   - Tlcreate (create-write-close micro-workloads)
//   - Tremove, Tmkdir, Tsetattr (file lifecycle metadata)
//   - Trename, Tsymlink, Tmknod (common structural metadata)
//
// NOT_CACHED: Trenameat, Tunlinkat, Tlink, Txattrwalk, Txattrcreate,
// Tlock, Tgetlock - cold-path ops with zero allocs in the bench
// suite.
var (
...
// between Put and Get, a concurrent peer could observe the stale
// pointer, so the Put-side clear is belt-and-braces.
//
// Twalk.Names is NOT cleared here: DecodeFrom rebuilds it via `make`,
// and TestTwalkReuseDoesNotAliasStrings locks that invariant into CI
// via unsafe.SliceData identity.
//
// Tlcreate.Name, Tmkdir.Name, Tremove.Fid etc. are scalar or Go
// strings (immutable backing store) - Get-side zero-reset covers
// them.
