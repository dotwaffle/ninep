...
// on unix domain sockets. The shipped proto.Payloader path in
// server/conn.go:sendResponseInline routes through
// net.Buffers.WriteTo, which on *net.UnixConn type-asserts to
// buffersWriter and calls pfd.Writev instead of a sequential Write
// loop. The strace log is the only defense-in-depth evidence that the
// kernel actually received a writev syscall (not a split of multiple
// write() calls).
//
// Design decisions:
//   - The test is implemented as a re-exec helper under strace -e
//     writev so the parent process can scan the syscall log for an
//     iovcnt of 2 or 3.
//   - The entire file is gated behind //go:build linux - non-linux
//     platforms do not compile this code. The runtime skip path is a
//     secondary gate for linux hosts without strace or with a
//     restrictive ptrace_scope.
//
// The iovcnt can be 2 or 3 depending on whether the Rread payload slot
// is non-empty. The shipped path emits 3 iovecs (hdr, fixedBody=4B
// count, payload) when len(payload) > 0, and 2 iovecs (hdr,
// fixedBody) when len(payload) == 0. The child workload issues a
// single Tread with Count=4096 against a known-populated 128 MiB file
// to guarantee a non-empty payload, but the log grep accepts either
// "], 2)" or "], 3)" so the test is resilient to a future collapse of
// hdr||fixedBody into a single iovec (would drop iovcnt to 2 on the
// same request).
//
// CI environments with kernel.yama.ptrace_scope >= 2 block strace
// attach. The test degrades to t.Skipf (not t.Fatalf) on
// "Operation not permitted" or "ptrace:" output so the test is
// well-behaved under restricted ptrace.
func TestPayloaderUsesWritev(t *testing.T) {
	...
	// start - strings.Contains is the correct matcher.
	...
	for scanner.Scan() {
		...
		// Accept either "], 2)" or "], 3)": iovcnt may drop to 2 if
		// hdr||fixedBody collapse is optimised in a future pass.
